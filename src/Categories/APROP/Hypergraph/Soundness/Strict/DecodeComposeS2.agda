...
--------------------------------------------------------------------------------
-- The strict decoder `∘`-SHAPE (part (I)ˢ), Stage 2.
--
-- Builds on the green Stage-1 `Strict.DecodeComposeS` (§0 `permuteˢ-X`,
-- §1 `RunBlocks`).  This module discharges the obstruction map left there:
--
--   (A)  `TermEmbedˢ` — the strict port of
--        `ProcessEdgesTermShape.TermEmbed.process-edges-term-emb`: the
--        per-edge + iterated RELABELLING-EQUIVARIANCE twin, parameterised by
--        an injective, label-preserving vertex embedding `(φ, ψ)`.  Both
--        residuals of the FIRE case are discharged:
--          • BOX factor (`box-emb`)  — `gen-cast` + `ψ-elab`;
--          • PERMUTE factor (`perm-emb`) — §0 `permuteˢ-X` (both sides) +
--            `permˢ-K X id` driven by `eval-coincide`.
--        SKIP/FIRE lock-step reuses `extract-prefix-via-injective-{just,
--        nothing}` exactly as the non-strict `extract-prefix-J-{just,nothing}`.
--
-- The non-strict original spans ~477 LOC (`ProcessEdgesTermShape`); the
-- strict edge-step has NO `unflatten`/`fire-mid`/`box-of` indirection (the
-- box is `genˢ (elab e) ⊗ˢ idˢ` directly, the permute a bare `permuteˢ`
-- under two `map-++` casts), so the port is materially shorter.
--------------------------------------------------------------------------------

open import Categories.APROP
open import Relation.Binary using (DecidableEquality)

module Categories.APROP.Hypergraph.Soundness.Strict.DecodeComposeS2
  (sig : APROPSignature)
...
open import Categories.PermuteCoherence.Map using (≈-fb-of-≡)
