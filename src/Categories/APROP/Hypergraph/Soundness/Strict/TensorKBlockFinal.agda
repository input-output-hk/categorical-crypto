...
-- `TensorBraidS.Braid` — from the RESERVOIR-THREADED K-block factorization
...
module Categories.APROP.Hypergraph.Soundness.Strict.TensorKBlockFinal
  (sig : APROPSignature)
...
module _
  ...
  module _ {A B C D : ObjTerm} (f : HomTerm A B) (g : HomTerm C D) where
    ...
    private
      ...
      -- `TensorBraidS.Braid.g-disjoint` / `KBlockDisjoint` on the G-side:
...
-- EXACT type of `PartI`'s / `SoundnessAssembly`'s `decodePˢ-⊗` parameter, so
