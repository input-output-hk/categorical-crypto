...
-- (a verbatim factor-out of the copy inside `DecodeComposeS3.ComposeShape`).
--
...
--
-- Pulled out so BOTH the `∘`-shape (`DecodeComposeS3`) and the ⊗-shape
-- (`TensorBraidS`/`TensorKBlockFinal`) can consume the SAME relabel for the
...
module Categories.APROP.Hypergraph.Soundness.Strict.TensorPVVRelabel
  (sig : APROPSignature)
