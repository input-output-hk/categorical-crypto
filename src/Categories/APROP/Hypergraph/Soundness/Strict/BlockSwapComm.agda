...
-- `Strict/DecodeSigmaS.agda` (the parameter `BSwapσ : Scr.BswapSig`).
--
--   block-swap-comm
--     : ∀ (L R : List V)
--     → permuteˢ (bswap L R)
...
-- companion lemma and the `[]` base cases proven in `DecodeSigmaS`.
--
...
module Categories.APROP.Hypergraph.Soundness.Strict.BlockSwapComm
  (sig : APROPSignature)
