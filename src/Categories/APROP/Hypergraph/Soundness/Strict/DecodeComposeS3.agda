...
--   * `process-edges-equivariantˢ` (StackEquivS): the K-side equivariance
--     keystone;
--   * `run-split-atˢ` (DecodeComposeS §1): the run-split;
...
-- caller's `Decoder`/`DecodeS` boundary casts: the boundary objects align
...
module Categories.APROP.Hypergraph.Soundness.Strict.DecodeComposeS3
  (sig : APROPSignature)
...
open import Categories.APROP.Hypergraph.Soundness.Strict.StackEquivS sig _≟X_ public
...
module ComposeShape {A B C₀ : ObjTerm} (g : HomTerm B C₀) (f : HomTerm A B) where
  ...
  pvv-relabelˢ {nH} {nJ} φ vJ vH veq {xs} {ys} p P Q =
    ...
    where
      mpJd = mp (Fin nJ) vJ (map φ xs)
      mpJc = mp (Fin nJ) vJ (map φ ys)
      mpHd = mp (Fin nH) vH xs
      mpHc = mp (Fin nH) vH ys
  ...
  private
    ...
    gperm' =
      ...
      (≈-trans (≡⇒≈ˢ (cast-irrel
                        ...
                        Pcod
                        (RC.permuteˢ (PermProp.map⁺ injL perm-f))))
        (pvv-relabelˢ injL vlC G.vlab vlab-injL perm-f Pdom Pcod))))
      where
  ...
  private
    ...
    kperm' =
      ...
      (≈-trans (≡⇒≈ˢ (cast-irrel
                        ...
                        Pcod
                        (RC.permuteˢ (PermProp.map⁺ remapP perm-g))))
        (pvv-relabelˢ remapP vlC K.vlab remapP-vlab perm-g Pdom Pcod))))
      where
