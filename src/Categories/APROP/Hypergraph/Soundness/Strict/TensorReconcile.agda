...
--
-- The TARGET (verified verbatim against `DecodeTensorS.Tensor`'s `reconcileˢ`
...
--     This is the strict, tensor-level analogue of `DecodeSigmaS`'s `perm≈`
...
--
-- REMAINING RESIDUAL (precisely typed, NOT discharged here — see foot):
...
module Categories.APROP.Hypergraph.Soundness.Strict.TensorReconcile
  (sig : APROPSignature)
...
module _
  ...
  module Reconcile {A B C D : ObjTerm}
    ...
    -- `reconcileˢ` parameter of `DecodeTensorS.Tensor`, and that feeding it
...
--      — already PROVEN: `DecodeTensorS.BlockSplit.process-edgesˢ-++` (and the
--      cast-light `DecodeComposeS.run-split-atˢ`).
--
...
--      `DecodeTensorS.GBlock.G-block-frameˢ` (from `term-sepˢ`).  The G-run on
--      `map injL G.dom` is bridged to `decodePˢ f` through the cross-vertex
--      permute `DecodeComposeS.permuteˢ-X` (`Fin (G.nV+K.nV)` ↦ `Fin G.nV`).
--
...
--      with `Strict/Braid.strict-braid` + `DecodeSigmaS.σ-hexˢʳ`, exactly as
--      `DecodeSigmaS`'s `inner≈` uses `σ-block-≈`.
--
