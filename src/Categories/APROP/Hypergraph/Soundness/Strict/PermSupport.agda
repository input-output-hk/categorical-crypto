...
-- `_≈ˢ_`-equal terms under `permuteˢ`.  It is the ONLY input to the strict
-- chain that is not constructed here — threaded as a module PARAMETER and
-- discharged once, in a later phase, by transporting the non-strict K
-- kernel through the strictification boundary `Strict.Embed`.
--
...
module Categories.APROP.Hypergraph.Soundness.Strict.PermSupport
  (sig : APROPSignature)
