...
module Categories.APROP.Hypergraph.Soundness.Strict.TensorBraidS
  (sig : APROPSignature)
...
module _
  ...
  module Braid {A B C D : ObjTerm}
    ...
    private
      ...
      open import Categories.APROP.Hypergraph.Invariant sig using (range-++)
      ...
      open import Categories.APROP.Hypergraph.Soundness.Decode sig using (extract-elem)
...
--     `DecodeTensorS.GBlock.G-block-frameˢ` (⇐ `Decoder.term-sepˢ`).
...
--   * `Braid.braidˢ-from-kblock` — the run-split (`DecodeComposeS.run-split-atˢ`)
