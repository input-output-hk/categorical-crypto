...
--     `order-invariantˢ` (`IsoInvarianceConcreteS`, BUILT) with the strict
...
module Categories.APROP.Hypergraph.Soundness.Strict.DecodePRespIso
  (sig : APROPSignature)
...
module _ {A B : ObjTerm} (f : HomTerm A B) where
  private
    ...
    -- translations coincide where the decoder runs (as in DecodeS /
