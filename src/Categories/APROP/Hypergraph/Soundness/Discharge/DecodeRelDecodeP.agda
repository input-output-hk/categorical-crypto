...
--------------------------------------------------------------------------------
-- Part (I) of the soundness proof: structural ↔ pruned-algorithmic
...
--        from the SAME shared residual records the unpruned soundness
...
open import Categories.APROP.Hypergraph.Soundness.Discharge.DecodeShape sig
  using (DecodeShapeResiduals; module DecodeShapeResiduals)
-- The shape lemmas (∘-side, ⊗-side), each in a top-level `module _ (objUIP)(Kf)`.
import Categories.APROP.Hypergraph.Soundness.Discharge.Sub.DecodeComposeShape sig as DCS
...
open import Categories.APROP.Hypergraph.Soundness.Discharge.DecodeRoundtripAgenSigma sig
