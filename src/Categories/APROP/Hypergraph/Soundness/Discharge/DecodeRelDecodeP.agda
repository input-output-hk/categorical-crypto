...
-- DIRECTLY, with NO reference to the unpruned decoder at all.  Uses ONLY:
--   * the atomic collapses (DAS) `decodeP X ≈ bridge X` (stated over the
--     pruned decoder; pruning only changes `∘`, so the atomic clauses are
--     the same as the historical unpruned ones);
--   * the unitor object-induction (`Cases.FromShape`/`Rho.FromShape`)
--     over the pruned ⊗-shape (DTP);
...
-- `ProcessEdgesTermShape.Assemble` bridge: the direct route makes the
-- whole unpruned decoder universe dead (see docs/size-reduction-strategies.md,
-- Lever 2 + the 2026-06-10 addendum).
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- The `ρ`-shape residual (pure boundary-subst₂ algebra over `decodeP`;
-- on `ρ⇒`/`ρ⇐`/`id` the pruned decoder reduces through the same
-- `decode-attempt-hId` clauses as the historical unpruned one).
private
  rho⇒-shape
    : ∀ A → decodeP (ρ⇒ {A})
...
module Wired
  ...
  -- The PRUNED ⊗-shape, packed into the opaque `Ty-⊗-shape` carrier
  -- (for the unitor inductions).
  ty-⊗-shape : Ty-⊗-shape
  ty-⊗-shape =
    unapply-⊗-shape (λ {A} {B} {C} {D} f g → DTP.decodeP-⊗-shape objUIP K f g)

  module CasesShape = Cases-sig.FromShape ty-⊗-shape
  module RhoShape   = Rho-sig.FromShape ty-⊗-shape rhoShapeResidual

  -- Atomic agreements (decode-rel X ≈Term decodeP X).  decode-rel X = bridge X
  -- definitionally, so each is `≈-Term-sym` of the collapse / the FromShape
  -- view.
  drd-Agen : ∀ {A B} (g : mor A B) → decode-rel (Agen g) ≈Term decodeP (Agen g)
  drd-Agen g = ≈-Term-sym (DAS.decode-Agen-collapse objUIP K g)

  drd-σ : ∀ {A B} ⦃ s : Symm ≤ Symm ⦄
        → decode-rel (σ {A = A} {B = B} ⦃ s ⦄) ≈Term decodeP (σ {A = A} {B = B} ⦃ s ⦄)
  drd-σ ⦃ s ⦄ = ≈-Term-sym (DAS.decode-σ-collapse objUIP K ⦃ s ⦄)

  drd-α⇒ : ∀ {A B C} → decode-rel (α⇒ {A} {B} {C}) ≈Term decodeP (α⇒ {A} {B} {C})
  drd-α⇒ {A} {B} {C} = ≈-Term-sym (DAS.decode-α⇒-collapse objUIP K {A} {B} {C})

  drd-α⇐ : ∀ {A B C} → decode-rel (α⇐ {A} {B} {C}) ≈Term decodeP (α⇐ {A} {B} {C})
  drd-α⇐ {A} {B} {C} = ≈-Term-sym (DAS.decode-α⇐-collapse objUIP K {A} {B} {C})

  -- The unitor agreements, via the FromShape views.  These views already
  -- produce `decode-rel X ≈Term decodeP X` (the field type stripped of
  -- the abstract Ty-X carrier is exactly that).
  open import Categories.APROP.Hypergraph.Soundness.DecoderAgreementSafe sig
    using ( apply-id; apply-λ⇒; apply-λ⇐; apply-ρ⇒; apply-ρ⇐ )

  drd-id : ∀ {A} → decode-rel (id {A}) ≈Term decodeP (id {A})
  drd-id = apply-id CasesShape.ty-id

  drd-λ⇒ : ∀ {A} → decode-rel (λ⇒ {A}) ≈Term decodeP (λ⇒ {A})
  drd-λ⇒ = apply-λ⇒ CasesShape.ty-λ⇒

  drd-λ⇐ : ∀ {A} → decode-rel (λ⇐ {A}) ≈Term decodeP (λ⇐ {A})
  drd-λ⇐ = apply-λ⇐ CasesShape.ty-λ⇐

  drd-ρ⇒ : ∀ {A} → decode-rel (ρ⇒ {A}) ≈Term decodeP (ρ⇒ {A})
