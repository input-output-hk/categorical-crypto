...
--------------------------------------------------------------------------------
-- Per-axiom soundness proofs that are fully constructive (no postulates).
--
-- Originally part of `Categories.APROP.Hypergraph.SoundnessAxioms`; split
-- out so this module can be `--safe` while the axiom-dependent proofs
-- (ρ⇒∘ρ⇐, α⇒∘α⇐, plus the five still-unproven axioms) stay there.
--
-- Everything here is `--safe`: the generic identity-composition isos
-- (`hCompose-hId-R-iso-generic`, `hCompose-hId-L-iso-generic`), the
-- tensor-identity iso (`hTensor-hEmpty-G-iso`), and the constructive
-- proofs of the eleven `-sound` lemmas that don't rely on any of the
-- focused postulates in `SoundnessAxioms`:
--
--   * `idˡ-sound`, `idʳ-sound`
...
--   * `σ∘σ-sound`
--
-- (`ρ⇒∘ρ⇐-sound` and `α⇒∘α⇐-sound` remain in `SoundnessAxioms` because
-- they depend on the `hTensor-G-hEmpty-iso-substed` and
-- `subst₂-hId-assoc-cancel` postulates.)
--------------------------------------------------------------------------------
