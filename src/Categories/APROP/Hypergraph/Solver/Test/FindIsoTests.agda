...
test-idˡ = soundness (from-just (findIso ⟪ id ∘ Agen f ⟫ ⟪ Agen f ⟫))

test-idʳ : Agen f ∘ id ≈Term Agen f
test-idʳ = soundness (from-just (findIso ⟪ Agen f ∘ id ⟫ ⟪ Agen f ⟫))

test-assoc : (Agen h ∘ Agen g) ∘ Agen f ≈Term Agen h ∘ (Agen g ∘ Agen f)
test-assoc = soundness
...
test-⊗-∘-dist = soundness (from-just (findIso
  ⟪ (Agen g ∘ Agen f) ⊗₁ (Agen f ∘ Agen h) ⟫
  ⟪ Agen g ⊗₁ Agen f ∘ Agen f ⊗₁ Agen h ⟫))
...
test-pentagon
  ...
  ≈Term α⇒ {a₀} {a₁} {a₂ ⊗₀ a₀}
       ∘ α⇒ {a₀ ⊗₀ a₁} {a₂} {a₀}
test-pentagon = soundness (from-just (findIso
  ⟪ (id {a₀} ⊗₁ α⇒ {a₁} {a₂} {a₀})
       ∘ α⇒ {a₀} {a₁ ⊗₀ a₂} {a₀}
       ∘ (α⇒ {a₀} {a₁} {a₂} ⊗₁ id {a₀}) ⟫
  ⟪ α⇒ {a₀} {a₁} {a₂ ⊗₀ a₀}
...
test-hexagon = soundness (from-just (findIso
  ⟪ id {a₁} ⊗₁ σ ∘ α⇒ {a₁} {a₀} {a₂} ∘ σ ⊗₁ id {a₂} ⟫
