...
module Categories.APROP.Hypergraph.Solver.Rewrite.DeepProv (sig-dec : APROPSignatureDec) where
...
module BuildO (L S : Hypergraph FlatGen) (emb : L ↪ᴴ S) where
  ...
  -- The carved graph's per-edge origins, in its (kahn) edge order.  Uses the
  -- shared `Rewrite.Kahn` engine instantiated at `E = EdgeO`, the same one
  -- `Deep.Build` runs at `E = Edge`; sharing the engine structurally
  -- guarantees both emit edges in the identical order — exactly the fact the
  -- provenance prediction relies on.
  originsO : Maybe (List (Maybe (Fin S.nE)))
  originsO = Maybe.map (map EdgeO.origin)
               (kahn EdgeO.ins EdgeO.outs (suc (length pending)) S.dom pending)
