...
-- generator is applied LAST).  This is the OPPOSITE of stdlib's
-- `Transposition.List.eval`, whose `_∘ₚ_` is diagrammatic and so applies
-- its head transposition FIRST.
------------------------------------------------------------------------
...
-- (both are insertion-sort connectivity arguments over adjacent
-- transpositions).  Unification examined and declined: the side conditions do
-- not align at the type level (`remove`/`rotate` here vs `remove`/incomparability
-- there), so a shared formalization would cost more than it saves.
...
-- here; `LehmerRotate.canonW-cons-rotate` re-derives the word-level peel
-- instead, from `inv-rotate-fb-0` + `lift~`/`++c-r`.
...
-- relations of `Sₙ`, presented AT THE FRONT only — that is what keeps the
-- constructor set minimal.  Propagation to arbitrary depth is then DERIVED
-- in §8 as `lift~`.  This is the word-level shadow of `_≅↭ⁱ_`.
--
...
-- `Coxeter.EvalSoundness` identities (imported as `Snd`).
...
-- generator follows from the corresponding `Coxeter.EvalSoundness` (`Snd`)
-- identity, lifted to its position by induction through the `cons-fb`
-- functor laws.
