...
-- SolverCompare) lives in the *wire-list* world, where the tensor of flat
...
-- reflectF, bridgeF, solveF) lives in `Categories.SolverFrontendCore`
...
-- machine-checked in `Categories.SolverFrontendTests`.
--------------------------------------------------------------------------------
...
module Frontend
  ...
  module Decide
    ...
    module Into
      ...
      private module IC = IntoCore Mon _≟X_ GenF decide?F C (λ where ⦃ () ⦄) ⟦_⟧ᵖ₀
...
-- Usage in `SolverFrontendTests.Target`.
--------------------------------------------------------------------------------
