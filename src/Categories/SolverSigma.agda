...
module Sigma {X : Set} (_≟X_ : DecidableEquality X)
  ...
  -- `v≤v`/`M≤S` are not global instances (see `Categories.FreeMonoidal`), so the
  -- instance-gated `σ` at the concrete `v = Symm` here needs a local witness.
  -- Kept `private` so it does not leak into importers (the APROP cone supplies
  -- its own `Symm ≤ Symm` and would otherwise see an ambiguous search).
  private
    instance
      Symm≤Symm : Symm ≤ Symm
      Symm≤Symm = v≤v
