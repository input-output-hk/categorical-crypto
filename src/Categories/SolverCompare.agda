...
module SolverCompareI
  ...
  module Decide
    ...
    -- (public: re-used by the standalone `Categories.SolverMor` showcase.)
    ≈NF⇒width : ∀ {n n'} {d : DiagU n} {d' : DiagU n'} → d ≈NF d' → n ≡ n'
    ≈NF⇒width nf[]        = refl
    ≈NF⇒width (nf∷ _ _)   = refl

    -- Width coercion of a flat HomTerm along a codomain-index equality.
    -- (public: re-used by `Categories.SolverMor` to state soundness in `coeW` form.)
