...
-- The Kleisli category of a `DiscreteMonad` at the discrete objects — TYPES and
-- maps `A → M B` up to pointwise `_≈ᴹ_` — with the cartesian tensor.
-- `Categories.Monad.Setoids.Discrete.Kleisliᴹ` is the same category built from a
-- setoid-wide triple; this presentation is spelled out with `_>>=_` instead of
-- `μ ∘ F₁` so that it computes on closed inputs, and it carries the monoidal
-- structure.
--
...
module Categories.Category.Kleisli.Discrete {ℓ} (Mo : DiscreteMonad ℓ) where
