...
open import Categories.Coherence.Monoidal.Compare using (module SolverCompareI)
open import Categories.Coherence.Monoidal.Normalize using (module NormalizeI)
open import Categories.Coherence.Monoidal.Reflect using (module ReflectI; module DecideCore)
...
module Sigma {X : Set} (_≟X_ : DecidableEquality X)
  ...
  module Decide
    ...
    -- the computing hit-witness (normalizes to ⊤ exactly on a hit); relied on
    -- by the `SigmaTests` witnesses below.
...
module SigmaTests where
  ...
  -- Fin-indexed signature (cf. Test.Frontend): decidable equality and the rank
