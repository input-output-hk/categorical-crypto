...
-- The user-facing convenience wrappers over the monoidal solver front-ends:
-- `Structural` (MacLane coherence) and `Mor` (monoidal morphism solver,
-- `solveMor!`).  Each packages a `FinSetup` / `Solver` instantiation into a
-- single vector-of-triples signature.
--
-- For goals in a *symmetric* monoidal target (braidings allowed) use
-- `Categories.Coherence.Symmetric` instead: its hypergraph front-end
-- (`solveH!`, `rewriteH!`, …) is a complete decision procedure for free-SMC
-- equations, and takes the same shape of input (atom alphabet with decidable
-- equality, `Fin`-indexed arity table, generator-interpretation table).
--
