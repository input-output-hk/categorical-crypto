...
-- `encode` and its injectivity `encode-inj` sidesteps that `--without-K`-safely;
-- `encode-inj` is stated at arbitrary endpoints, which is where the one
-- unavoidable use of Hedberg UIP on `List X` lives.
--------------------------------------------------------------------------------
...
module CompareI {X : Set}
  ...
  module Decide ⦃ _ : DecEq Gen ⦄ where
    ...
    -- Deciding the encoded layer lists avoids every match against the
    -- `++`-composite indices.
