...
-- to run, and that cost is the thing under test.  The goal is left to
-- inference: written out in `C`'s own vocabulary it exhausts the default 3 GiB
-- heap from n = 10 on.  `Test.Frontend` states the ascribed form, at its own
-- (small) sizes.
--------------------------------------------------------------------------------
