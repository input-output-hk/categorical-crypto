...
-- instance would clash with the APROP cone's local `Symm ≤ v` instances); these
-- σ tests build free `σ` terms at the `Symm` variant, so supply it locally.
-- (No type signature: it fixes the `_≤_` ambiguity with `Data.Fin._≤_`.)
