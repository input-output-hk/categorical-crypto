...
--------------------------------------------------------------------------------
-- Tests for the SYMMETRIC solver front-end (`Categories.Coherence.Monoidal.Frontend.Sigma`).
--
...
--   * `Target`     — C-level showcase through the public `Symmetric` entry
...
open import Categories.Coherence.Monoidal.Frontend.Sigma using (module FrontendS)
...
------------------------------------------------------------------------
-- C-level showcase through the public `Symmetric` entry: statements read in the target's
...
module Target {o ℓ e : Level}
  ...
  module At
    ...
    open FreeMonoidalHelper Symm (Fin 2) using () renaming (Var to V; _⊗₀_ to _⊗ᵒ_)
    open Coh.Symmetric C Sym (A ∷ B ∷ [])
         ( ((V zero , V zero)             , sᴹ)            -- gen 0 : A → A ↦ sᴹ
         ∷ ((V (suc zero) , V (suc zero)) , tᴹ)            -- gen 1 : B → B ↦ tᴹ
         ∷ [] )
      using (gen; solveMorσ!; rewriteMorσAuto!) renaming (module S to Sσ)
    ...
    σC : C .MonoidalCategory.U [ A MC.⊗₀ B , B MC.⊗₀ A ]
