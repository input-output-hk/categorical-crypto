...
open import categorical-crypto.Prelude hiding (_∘_; id; map; merge; _>>=_; Ordering)

open import Data.Bool using (if_then_else_)
open import Data.List.Properties using (≡-dec; ++-assoc)
open import Data.Maybe as Maybe using (_>>=_; _<∣>_)
...
module NormalizeI {v : Variant} {X : Set} (E : WireEngine v {X})
  ...
  -- (soundness `⟦substDiagU⟧`).  Exercised black-box by the `Reorder` litmus in
  -- `Categories.Coherence.Monoidal.Test.Frontend`.
  ...
  -- `⟦ input ⟧ ≈Term ⟦ swapped ⟧` is the honest per-swap soundness (exercised
  -- black-box by `Reorder` in `Categories.Coherence.Monoidal.Test.Frontend`).
