...
open import Categories.Coherence.Monoidal.WireCoherence using (module WireCoh)
...
module WireSig (v : Variant) {X : Set} (Mor : List X → List X → Set) where
  ...
  -- can be built from them (e.g. the σ-conjugated block braiding of Sigma).
...
-- interpretation `⟦box⟧` into the flat free category.  Bundles the telescope
-- shared verbatim by every wire engine (UntypedI / ReflectI / NormalizeI /
-- SolverCompareI / DecideCore / FBridge); the `↑` vocabulary `⟦box⟧`'s type
-- needs is derived here once.  `_≟X_` is kept a separate argument since the
-- base `UntypedI` does not need decidable equality.
record WireEngine (v : Variant) {X : Set} : Set₁ where
  field Mor : List X → List X → Set
  open WireSig v {X} Mor public using () renaming (wires to wires↑; mor to mor↑)
  open FreeMonoidalHelper.Mor v X mor↑ public using () renaming (HomTerm to HomTerm↑)
...
module UntypedI {v : Variant} {X : Set} (E : WireEngine v {X}) where
  ...
  module TwoBoxSwap (pre mid r : List X) {a₁ b₁ a₂ b₂ : List X}
    ...
    -- with one offset-parametrised lemma each — see Normalize §11d').
    reassocF : (x : List X)
