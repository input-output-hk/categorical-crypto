...
-- (`Categories.Coherence.Monoidal.MorSolve`).
--
-- Requires `standard-library-meta` at branch `algebraic-solvers`
-- (the generic solver frontends `Tactic.Solver.Core` with
-- `AtomEmission` and `TwoLevelFrontend`), which is what `flake.nix`
-- pins.
--
...
--
-- The macro synthesises everything the manual call sites write by
-- hand today:
--   * the object-atom vector `vars` (each distinct object atom found
--     in leaf implicits and generator types becomes `V i`);
--   * the generator signature `gens`: one ((src , tgt) , morphism)
--     triple per distinct box morphism, its arities obtained by
--     `inferType` on the atom and parsing the resulting hom type;
--   * both sides as intrinsically-typed `HomTerm`s over the
--     signature (`gen i` = `var (genS i)` referencing the store
--     index).
--
-- Both levels ride on the generic frontends: the OBJECT level is an
-- ordinary `DetectedTheory` (operators `_⊗₀_`, its `F₀ (⊗ M)`
-- unfolding, and `unit`; atoms in store-indexed mode,
-- `AtomEmission.indexed`), and the MORPHISM level is the two-level
-- theory on top of it (`Tactic.Solver.Core.TwoLevelFrontend`):
-- `_∘_`/`_⊗₁_` as
-- plain operators, `id` and the projected unitor/associator isos as
-- leaves whose hidden object arguments are the index terms, box atoms
-- referenced as `gen i`, and the signature synthesised from the
-- boxes' inferred hom types. The `prepIndex` hook is `expandObj`,
-- which rebuilds elaborator-produced `F₀`-of-derived-tensor endpoints
-- onto the `_⊗₀_` shape the pure object operators recognise;
-- `f₁parse` is its morphism-level mirror, reading `F₁ (⊗ M) ⟨pair⟩` —
-- a derived tensor functor's action, stuck on the blocked `⊗` — as
-- the `_⊗₁_` node on the pair's (whnf'd) components.
--
--   * Recognition happens on canonical `Monoidal.*` / `Category.*`
--     heads after whnf normalises user aliases onto them; leaf
--     HomTerm constructors get their ObjTerm indices supplied
--     explicitly, composite nodes leave them to the elaborator.
--   * Box-atom identity is the original spelling only (α-equality);
--     object atoms get the store's dual (orig , whnf) key.
--   * The macro never `commitTC`s before the driver's blocking points
--     (see `TwoLevelFrontend.solveTwoLevelEquation`) — committing earlier
--     makes two blocked call sites in one declaration wake each other
--     forever.
...
open import Data.Fin.Reflection using (toTerm)

open import Reflection
open import Reflection.AST.Argument
open import Reflection.AST.Term using (_⋯⟅∷⟆_; _⋯⟨∷⟩_)
open import Reflection.TCM.Syntax
open import Agda.Builtin.Reflection using (primQNameEquality)

open import Reflection.Utils.Args using (vArgs; takeLast)
open import Reflection.Utils.Core using (headName)
open import Reflection.Utils.Reduction using (whnfIfReducible)
open import Reflection.QuotedDefinitions using (_`,_; `vec)
...
open import Categories.FreeMonoidal using (module FreeMonoidalHelper)
open import Categories.Coherence.Monoidal.Frontend.Core using (module FinSig)
import Categories.Coherence.Monoidal as Coh

------------------------------------------------------------------------
-- Canonical names, all obtained by plain `quote`. Parameterized
-- modules (`FreeMonoidalHelper`, `FinSig`, `Categories.Morphism`) are
-- accessed WITHOUT instantiating them — lifted use resolves to the
-- original names, whereas quoting through a module application would
-- yield the application's parameter-baked copies.

private
  -- recognition points (goal side, agda-categories vocabulary)
  `∘    = quote Category._∘_
  ...
  -- `_⊗₀_ = curry′ F₀`, so elaborator-inferred objects surface as
  -- `Functor.F₀ (Monoidal.⊗ M) (X , Y)` — recognised alongside `_⊗₀_`;
  -- `F₁` is the same story one level up (derived tensor functors).
  -- `₀`/`₁` are agda-categories' record-module shorthands for the same
  -- projections; functor combinators (`∘F`, `※`, …) unfold onto them,
  -- so both spellings occur and each parser recognises both heads.
  `F₀   = quote Functor.F₀
  `₀    = quote Functor.₀
  `F₁   = quote Functor.F₁
  `₁    = quote Functor.₁
  ...
  -- emission targets (backend vocabulary)
...
------------------------------------------------------------------------
-- The OBJECT level: an ordinary `DetectedTheory` driven by
-- `parseGoalTerm`, with atoms emitted in `indexed` mode as `Var i`
-- references into the store that later becomes the `vars` vector.
