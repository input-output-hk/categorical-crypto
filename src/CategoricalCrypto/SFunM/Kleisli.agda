...
-- The stateful-function machine layer over a Kleisli triple on `Setoids`.
-- The sibling `CategoricalCrypto.SFunM` is the SAME construction over a
-- level-polymorphic `⦃ Monad M ⦄ ⦃ MonadSetoid M ⦄`; neither parametrisation
-- subsumes the other (`Type↑` is level-polymorphic, a `KleisliTriple (Setoids
-- ℓ ℓ)` is not), so both are kept: the crypto layer runs on the sibling, and
-- the `Spike` certifications (`Spike.Instance.{Setoid,Type}`) run on this one.
...
module CategoricalCrypto.SFunM.Kleisli (K : KleisliTriple (Setoids 0ℓ 0ℓ)) where
