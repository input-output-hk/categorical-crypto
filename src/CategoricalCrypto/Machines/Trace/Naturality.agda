...
opaque
  ...
  trace-∘ˡ {A} {B} {C} {X} g f =
    ...
    where
      ...
      d-ok : discard (state g ⊛ state f) ∘ (ρ⇒ ⊗₁ id)
           ≈ discard ((state g ⊛ Iˢ) ⊛ state f)
      d-ok = ⊛-discard (state g ⊛ Iˢ) (state g) (state f) (ρ-discard (state g))

      p-ok : (ρ⇒ ⊗₁ id) ∘ point ((state g ⊛ Iˢ) ⊛ state f)
           ≈ point (state g ⊛ state f)
  ...
  trace-∘ʳ {A} {B} {C} {X} f h =
    ...
    where
      ...
      d-ok : discard (state f ⊛ state h) ∘ (id ⊗₁ ρ⇒)
           ≈ discard (state f ⊛ (state h ⊛ Iˢ))
      d-ok = ⊛-discardʳ (state f) (state h ⊛ Iˢ) (state h) (ρ-discard (state h))

      p-ok : (id ⊗₁ ρ⇒) ∘ point (state f ⊛ (state h ⊛ Iˢ))
           ≈ point (state f ⊛ state h)
