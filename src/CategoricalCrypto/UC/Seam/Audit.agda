...
--
-- The other end — an audit-watching STRATEGY recognized as one of the ancilla
-- contexts the graded bound quantifies over, which is `UC.Seam`'s embedding one
-- grade up — is `AuditIsBounded` below, proved in `UC.Seam.Audit.Bounded`.
--
...
module TrivialGrade (𝟘 : Channel) (ι : (B : Iface) → ifaceᵒ B ⇒ T₀ 𝟘 (ifaceᵒ B)) where
  ...
  -- environment, IS one of the contexts `AuditBound` quantifies over, so the
  -- graded bound restricts to layer 1's own `Bounded`.  Proved at `𝟘 = unit`
  -- in `UC.Seam.Audit.Bounded`, whose header has the three ingredients.
  --
