...
-- model (§2) plus the RO/Merkle–Damgård artifacts (postulated at their exact
-- MerkleDamgard.agda types, modulo the ℚ/Dist stand-ins — the proofs behind
-- them already exist and carry over), the ε-absorption bridge, and the OP-level
-- protocol bridges, `MD ≤UC RO` is DERIVED from the §1 dummy-adversary theorem.
-- Co-consumption of the abstract theorem and the existing MD theorem in one
-- term is the plan's thesis.  Everything still open is bundled in `ROData`.
...
module RO
  ...
  open Coherence ℐ-machines using (Var)
  ...
  -- ── The RO/MD interface, postulated at its MerkleDamgard.agda types ─────────
  ...
  record ROData : Set (suc ℓr ⊔ ℓq ⊔ o ⊔ ℓ ⊔ cs ⊔ ℓs) where
    ...
    field
      ...
      U-ideal : U (pureAtk roSimulator ∘ᴼ ι roIdeal) ≈ℰ General-M
