...
-- ============================================================================
-- Two three-fold rearrangements for `Pair`.
--
-- `CategoricalCrypto.Machine.Reindex` needs the middle-four exchange; the
-- Kleisli composition's forwarder needs this three-leaf cousin, which moves
-- the third component up beside the first and pushes the second to the right:
--
--     Pair M₁ (Pair M₂ M₃)  ≅ᴹ  Pair (Pair M₁ M₃) M₂   (up to relabelling)
--
-- and the monoidal associator needs plain reassociation:
--
--     Pair M₁ (Pair M₂ M₃)  ≅ᴹ  Pair (Pair M₁ M₂) M₃   (up to relabelling)
--
...
opaque
  ...
  -- ------------------------------------------------------------------------
  -- Reassociation.  Same recipe as the rotation: the leaves are re-tagged, the
  -- middle one moving from the right subtree to the left.
  -- ------------------------------------------------------------------------
