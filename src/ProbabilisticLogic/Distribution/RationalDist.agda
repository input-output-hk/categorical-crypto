...
--
-- `Dist-ℚ A` is the type of probability distributions on `A`: ℚ-valued weights
-- that are non-negative and sum to 1.  Both are invariants of the BUNDLE only —
-- the raw carrier `DistData` stays unconstrained, so intermediate computations
-- (`scale-ℚ`, `bindᴰ-cons`, …) need re-establish nothing.

open import categorical-crypto.Prelude
open import Data.Rational as ℚ using (ℚ; 0ℚ; 1ℚ)
open import Data.Rational.Properties
  using ( +-*-commutativeRing
        ; +-identityˡ; +-identityʳ; +-assoc; +-mono-≤
        ; *-identityˡ; *-identityʳ; *-zeroʳ; *-assoc; *-monoˡ-≤-nonNeg
        ; *-distribˡ-+; ≤-refl )
...
NonNeg-L : {A : Type ℓ} → List (ℚ × A) → Type ℓ
NonNeg-L xs = All.All (λ e → 0ℚ ℚ.≤ proj₁ e) xs

NonNegᴰ : {A : Type ℓ} → DistData A → Type ℓ
NonNegᴰ μ = NonNeg-L (NE.toList μ)

------------------------------------------------------------------------
-- Probability distribution: a `DistData` whose weights are non-negative
-- and sum to 1.
...
------------------------------------------------------------------------
-- Non-negativity lemmas, mirroring the mass family above: the `Dist-ℚ`
-- constructions below re-establish `weights-nn` through them.

nn-scale-L : {A : Type ℓ} (q : ℚ) → 0ℚ ℚ.≤ q → (xs : List (ℚ × A)) → NonNeg-L xs
           → NonNeg-L (map (λ (q′ , a) → (q ℚ.* q′ , a)) xs)
nn-scale-L q 0≤q []       All.[]         = All.[]
nn-scale-L q 0≤q (x ∷ xs) (0≤w All.∷ ws) = 0≤* 0≤q 0≤w All.∷ nn-scale-L q 0≤q xs ws

nn-scale-ℚ : (q : ℚ) → 0ℚ ℚ.≤ q → (μ : DistData A) → NonNegᴰ μ → NonNegᴰ (scale-ℚ q μ)
nn-scale-ℚ q 0≤q (h NE.∷ t) (0≤w All.∷ ws) = 0≤* 0≤q 0≤w All.∷ nn-scale-L q 0≤q t ws

nn-⁺++⁺ : (μ ν : DistData A) → NonNegᴰ μ → NonNegᴰ ν → NonNegᴰ (μ NE.⁺++⁺ ν)
nn-⁺++⁺ (h NE.∷ t) ν (0≤w All.∷ ws) ns = 0≤w All.∷ AllP.++⁺ ws ns

private
  nn-bindᴰ-cons : (e : ℚ × A) (rest : List (ℚ × A)) (k : A → DistData B)
                → NonNeg-L (e ∷ rest) → (∀ a → NonNegᴰ (k a))
                → NonNegᴰ (bindᴰ-cons e rest k)
  nn-bindᴰ-cons (q , a) []         k (0≤q All.∷ _)  hk = nn-scale-ℚ q 0≤q (k a) (hk a)
  nn-bindᴰ-cons (q , a) (e ∷ rest) k (0≤q All.∷ ws) hk =
    nn-⁺++⁺ (scale-ℚ q (k a)) (bindᴰ-cons e rest k)
            (nn-scale-ℚ q 0≤q (k a) (hk a)) (nn-bindᴰ-cons e rest k ws hk)

nn-bindᴰ : (μ : DistData A) (k : A → DistData B)
         → NonNegᴰ μ → (∀ a → NonNegᴰ (k a)) → NonNegᴰ (μ >>=ᴰ k)
nn-bindᴰ (e NE.∷ rest) k = nn-bindᴰ-cons e rest k
...
return-ℚ : A → Dist-ℚ A
return-ℚ a = mk-Dist ((1ℚ , a) NE.∷ []) (+-identityʳ 1ℚ) (0≤1ℚ All.∷ All.[])
...
μ >>=ᴹ k = mk-Dist (entries μ >>=ᴰ entries ∘ k)
                   ...
                   (nn-bindᴰ (entries μ) (entries ∘ k) (weights-nn μ) (weights-nn ∘ k))
...
-- Support
--
-- Which points a distribution actually charges is a property of the
-- REPRESENTATION only, so the monad operations preserve it with no
-- weight-positivity input.

OnSupportᴰ : (A → Type ℓ′) → DistData A → Type _
OnSupportᴰ P μ = All.All (λ e → P (proj₂ e)) (NE.toList μ)

OnSupport : (A → Type ℓ′) → Dist-ℚ A → Type _
OnSupport P μ = OnSupportᴰ P (entries μ)

private
  -- `scale-ℚ` and `_⁺++⁺_` rearrange weights only, so the predicate rides along.
  all-scale-L : {A : Type ℓ} {P : A → Type ℓ′} (q : ℚ) (xs : List (ℚ × A))
              → All.All (λ e → P (proj₂ e)) xs
              → All.All (λ e → P (proj₂ e)) (map (λ (q′ , a) → (q ℚ.* q′ , a)) xs)
  all-scale-L q []       All.[]       = All.[]
  all-scale-L q (x ∷ xs) (p All.∷ ps) = p All.∷ all-scale-L q xs ps

  all-scale : {P : A → Type ℓ′} (q : ℚ) (μ : DistData A)
            → OnSupportᴰ P μ → OnSupportᴰ P (scale-ℚ q μ)
  all-scale q (h NE.∷ t) (p All.∷ ps) = p All.∷ all-scale-L q t ps

  all-⁺++⁺ : {P : A → Type ℓ′} (μ ν : DistData A)
           → OnSupportᴰ P μ → OnSupportᴰ P ν → OnSupportᴰ P (μ NE.⁺++⁺ ν)
  all-⁺++⁺ (h NE.∷ t) ν (p All.∷ ps) qs = p All.∷ AllP.++⁺ ps qs

  all-bindᴰ-cons : {P : A → Type ℓ′} {Q : B → Type ℓ″}
                   (e : ℚ × A) (rest : List (ℚ × A)) (k : A → DistData B)
                 → All.All (λ x → P (proj₂ x)) (e ∷ rest)
                 → (∀ a → P a → OnSupportᴰ Q (k a))
                 → OnSupportᴰ Q (bindᴰ-cons e rest k)
  all-bindᴰ-cons (q , a) []         k (pa All.∷ _)  hk = all-scale q (k a) (hk a pa)
  all-bindᴰ-cons (q , a) (e ∷ rest) k (pa All.∷ ps) hk =
    all-⁺++⁺ (scale-ℚ q (k a)) (bindᴰ-cons e rest k)
             (all-scale q (k a) (hk a pa)) (all-bindᴰ-cons e rest k ps hk)

  all-bindᴰ : {P : A → Type ℓ′} {Q : B → Type ℓ″} (μ : DistData A) (k : A → DistData B)
            → OnSupportᴰ P μ → (∀ a → P a → OnSupportᴰ Q (k a))
            → OnSupportᴰ Q (μ >>=ᴰ k)
  all-bindᴰ (e NE.∷ rest) k = all-bindᴰ-cons e rest k

OnSupport-return : {P : A → Type ℓ′} {a : A} → P a → OnSupport P (return-ℚ a)
OnSupport-return pa = pa All.∷ All.[]

OnSupport-bind : {P : A → Type ℓ′} {Q : B → Type ℓ″}
...
-- Monotonicity
--
-- What `weights-nn` is for: a pointwise inequality on the support integrates,
-- because each weight scales it the right way round.  See
-- `…RationalDist.Expectation.E-mono-on` for the `Dist-ℚ`-level statement.

private
  lookup-L-mono : {P Q : A → ℚ} (xs : List (ℚ × A)) → NonNeg-L xs
                → All.All (λ e → P (proj₂ e) ℚ.≤ Q (proj₂ e)) xs
                → Lin.lookup-L xs P ℚ.≤ Lin.lookup-L xs Q
  lookup-L-mono []             All.[]         All.[]         = ≤-refl
  lookup-L-mono ((q , _) ∷ xs) (0≤q All.∷ ws) (le All.∷ les) =
    +-mono-≤ (*-monoˡ-≤-nonNeg q {{ℚ.nonNegative 0≤q}} le) (lookup-L-mono xs ws les)

lookupᴰℚ-mono : {P Q : A → ℚ} (μ : DistData A) → NonNegᴰ μ
              → OnSupportᴰ (λ a → P a ℚ.≤ Q a) μ
              → lookupᴰℚ μ P ℚ.≤ lookupᴰℚ μ Q
lookupᴰℚ-mono (h NE.∷ t) = lookup-L-mono (h ∷ t)
