...
>>=⊥-congʳ : (f : A → Dist⊥ B) (μ ν : Dist⊥ A)
           → μ ≈Mℚ ν → (μ >>=⊥ f) ≈Mℚ (ν >>=⊥ f)
>>=⊥-congʳ f μ ν μ≈ν = >>=⊥-cong {μ = μ} {ν} {f} {f} μ≈ν (λ a → Eq.refl {x = f a})

-- `Dmap just` embeds a total distribution; on a point mass it is `return⊥`.
dmap-ret : (x : A) → Dmap just (return-ℚ x) ≈Mℚ return⊥ x
dmap-ret x = >>=ᴹ-identityˡ x (return-ℚ ∘ just)

Dmapⱼ-cong : (μ ν : Dist-ℚ A) → μ ≈Mℚ ν → Dmap just μ ≈Mℚ Dmap just ν
Dmapⱼ-cong μ ν e = >>=ᴹ-cong {μ = μ} {ν} {return-ℚ ∘ just} {return-ℚ ∘ just} e
                     (λ a → Eq.refl {x = return-ℚ (just a)})
