...
module ProbabilisticLogic.Metric c ℓ (a : Abstract c ℓ) where

open Abstract a

private variable Ω : Type

record isSupremum {a} (T : Set a) (f : T → Probability) (p : Probability)
  : Type (a ⊔ˡ c ⊔ˡ ℓ) where
  field isUpperBound : ∀ {t} → f t ≤ p
        isLeastUpperBound : ∀ {q} → q < p → ∃[ t ] ¬ f t ≤ q

dTV_,_≡_ : (P Q : ProbDistr Ω) → (p : Probability) → Type (sucˡ lzero ⊔ˡ c ⊔ˡ ℓ)
