...
Both are discharged in `Examples/ROCommitment/Realization/Assembly.agda`; §6
records the proof and the schedule pins it carries.
...
**Corrupted receiver — `comSimʰ`** (`Examples/CoinToss/Compose.agda:155`),
mirroring `comSim` at rate `1` over `ROHU.simʰᵒ`
...
| `QB 1 simulatorʰ` → `Certified Lkʰᶠ Advʰᶠ` | ditto, receiver | EXISTS, `Hiding/UC.agda:134`, packaged at `Examples/CoinToss/Compose.agda:155` |
...
| `PolyQB Rcom` / `PolyQB Icom` | ONLY the `Canonicalᴺ._≤UC_` consequence | EXIST, `Examples/ROCommitment/Realization/Assembly.agda:62,65` (receiver twins `:69,72`) |
...
| F4 | The game bound holds only at `asks≤ m d` for a FINITE strategy `d : Strat Q R` | `Examples/ROCommitment/Game.agda:576` | THEOREM at the game; crossing to contexts is `dominatedᵍᵠ`, §7 B2-M5 |
...
**The slack that does work.** `dominatedᵒ`/`dominatedᵍᵠ` take `δ : ℚ` with
`0ℚ ℚ.< δ` chosen per instance (`UC/Model/Dominated.agda:119,149`), so it may
...
| 3. per-strategy → per-context | `dominatedᵍᵠ`: hypothesis at `asks≤ (ctxBudget c c′) d`, conclusion read at `ε (ctxBudget c c′)` — exactly `_≈ᵁᵠ[_]_`'s convention (`UC/Quantitative/Contextual.agda:58-62`) | identity |
...
## 6. The assembly proof — DELIVERED

`Examples/ROCommitment/Realization/Assembly.agda`, wired into the root
...
| B2-8 | the density lemma at a NONTRIVIAL grade | DONE and IN USE — `UC/Model/Dominated.agda:183 dominatedᵍ`, consumed at `Examples/CoinToss/Ideal/Receiver/Compose.agda:113` |
...
| B2-M5 | a BUDGETED `dominatedᵍ` | DELIVERED — `UC/Model/Dominated.agda:149 dominatedᵍᵠ`, hypothesis at `asks≤ (ctxBudget c c′) d`; the unitor's factor is taken back by `*-identityʳ`, and `dominatedᵍ` (`:183`) is now its instance at the unrestricted hypothesis, with its statement unchanged |
| B2-M6 | `graded-∘ᵒ : gradedᵒ f ∘ procᵒ g ≈ gradedᵒ (f 𝒫.∘ g)` at a SINGLE grade | DELIVERED — `UC/Model/Graded.agda:69`, `G.Equiv.refl`, beside `graded₂-∘ᵒ` |
| B2-M8 | `comSimʰ : Certified Lkʰᶠ Advʰᶠ` | DELIVERED — `Examples/CoinToss/Compose.agda:155`, beside `comSim` |
| B2-M9 | `PolyQB Rcom`, `PolyQB Icom` (and the receiver twins) | DELIVERED — `Examples/ROCommitment/Realization/Assembly.agda:62,65,69,72`, over the two wire certificates `Examples/ROCommitment/UC.agda:125 idealQB` and `Hiding/UC.agda:138 idealʰQB` |
| B2-M1 | `compose-settles` / `Settles-∘`: identify `𝒢ₚ`'s `_∘_` with a `traceᴹ` whose step has a kernel, so `trace-settles` applies to `real 𝒫.∘ resource` and to `subᴵ simulator 𝒫.∘ (ideal 𝒫.∘ resource)` | DELIVERED — `Protocol/Machine/Trace/Compose.agda`: `Kᴳ`/`kᴳ-settles` give `Machines.Collapse.kᴳ` a kernel, `ranked-from-kᴳ` reduces `Ranked` to that dispatch, `compose-settles` is `trace-settles` at it, and `compose-StepSettles`/`compose-pr` are the closed instance. `docs/fcom-uc.md` §1b is right that no `Settles` witness can ride the `S.≈ᴹ` to `𝒢ₚ`'s own `_∘_`; what it did not notice is that the CLOSED RUN's conclusion can — `ProbabilisticLogic.Dp.Settle.cum-settled-≈ₚ`, the escape hatch that section names and declines. So the identification is one `≈ₚ` at the end, and reading the two structural legs off `W.α`/`W.γ` is not needed at all |
| B2-M2 | `Ranked` instances at `real`, `ideal`, `subᴵ simulator`, and the two composites | DELIVERED — `Examples/ROCommitment/Realization/Machine.agda`: `rankedᴿ` (`real 𝒫.∘ resource`, rank 1/0, fuel 2), `rankedᴵⁿ` (`ideal 𝒫.∘ resource`, same), `rankedᴵ` (`subᴵ simulator` over that, rank `suc (Φˢ + Φˢ)` / `Φˢ + Φˢ`, fuel 4, over `Examples.ROCommitment.UC.Φˢ` — the simulator's own allowance ledger already counts what the rank counts). The step-table facts are `real-up`, `ideal-up`, `sub-up`; the generic support clauses they spend are new: `Dp.Support.Supp-bot`/`Supp-return`/`Supp-bind` |
| B2-M7 | the ask-preserving relabelling `Neg (Advᴵ ⊗ᴵ Honᴵ) ↔ Game.Q`, `Pos (Advᴵ ⊗ᴵ Honᴵ) ↪ Game.R` | DELIVERED — `Examples/ROCommitment/Realization/Bisim.agda`: `toQ`/`fromQ` with `toQ-fromQ`/`fromQ-toQ`, `toR` with its section `fromR` and `fromR-toR`, and `verdict-realOpen`. `idleR` is outside `toR`'s image and `fromR` sends it anywhere, which is sound because `prune deadR` removes exactly the activations that produce it. The strategy half is generic: `Strategy.mapStrat` with `asks≤-mapStrat` (one ask stays one ask) and `GamePlaying.Partial.runWith⊥-bisimʳ`, the alphabet-changing `runWith⊥-bisim` |

### Missing (verified absent)

| item | statement | verdict |
|---|---|---|
| B2-M3 | the example's two `StepBisim⊥`s against pruned kernels, with `deadR`/`deadI` naming the off-protocol activations | REAL SIDE DELIVERED — `Examples/ROCommitment/Realization/Bisim.agda`: `deadR`, `respR⊥ = prune deadR respR`, `_≋ᴿ_`, `bisimᴿ : StepBisim⊥ʳ toQ fromR Kᴿ respR⊥ _≋ᴿ_`, read through the trace by `enterᴿ`/`oracleᴿ`/`digest-relay`/`digest-check`. IDEAL SIDE IMPOSSIBLE at the machines as they stand: `simStep` conses `(x , d)` onto the simulator's log at EVERY relayed answer (`Examples/ROCommitment.agda:195`), including one the oracle served from its table, so after a repeated query the log holds the digest twice and `extract` reads `false` (`Bisim.extract-relog`) where `Game.comI`'s `extract c t` reads the committed bit (`Bisim.extract-table`) — and the game's bad event is `dup` of the ORACLE's answer log, which a repeat does not raise. `Examples/ROCommitment/Extraction.agda`'s header claim "the simulator's log IS the oracle's table" is false for this machine, and no state relation can repair it. Maintainer call: either `simStep` caches (only cons a point the log does not already answer) or `Game.respI` is restated over the relay log — the second makes `Game.cert` false, since a repeated query raises `dup` with certainty |
| B2-M4 | `binding-bound`: the two-sided `cum` bound between the real and simulated-ideal CLOSED machines at every `asks≤ m d` | BLOCKED by B2-M3's ideal side. The REAL leg is delivered — `Examples/ROCommitment/Realization/Bound.agda`: `real-run` (the closed machine's `cum` family reaches `Pr₁⊥ (runWith⊥ respR⊥ sR₀ (mapStrat toQ fromR d))` exactly), `game-bound⊥` (`Game.cert` through `prune-cert`, and the new `hop-boundᵇ⊥`/`badProb⊥-cong` because pruning the COUPLING and pruning its marginal are only `E⊥`-equal), and `binding-real`, which bounds the closed real MACHINE against the pruned ideal GAME by `Game.ε m` at every `asks≤ m d`. What is missing is only the replacement of that ideal game by the ideal machine |

### The route, once B2-M1..M4 and M7 are in

1. `binding-bound` (B2-M4) at `u = real 𝒫.∘ resource`,
   `v = subᴵ simulator 𝒫.∘ (ideal 𝒫.∘ resource)`, both `Proc unitᴵ (Advᴵ ⊗ᴵ Honᴵ)`.
2. `dominatedᵍᵠ` (B2-M5) at `P = Advᴵ`, `B = Honᴵ` → `_≈ᵁᵠ[ εᴿ n ]_`
...
1. **`docs/fcom-uc.md` took `δ = εᶜ n q`, unsound at zero allowance** —
   `εᶜ n 0 = 0ℚ`, and `ctxBudget c c′ = 0` whenever `c = 0`, which
   `_≈ctx[_]_`'s `{c c′ : ℕ}` does not exclude. FIXED: that section now takes
   §5's `δ = inv-pow-2 n`, as source already does at the receiver hop.
2. **`docs/fcom-uc.md` called re-opening the grade "not proved, and not
   obviously cheap"** — `UC/Model/Dominated.agda:183 dominatedᵍ` is exactly
   that, with a live consumer at
   `Examples/CoinToss/Ideal/Receiver/Compose.agda:113`. FIXED, and what was
   actually missing — the budgeted variant — is B2-M5, delivered.
3. **`docs/coin-toss.md` cited `UC/Asymptotic/Compose.agda:219,251`
   for `≈ctx-dom`/`≤UC^ωᵉ-dom`** — that module is 13 lines and re-exports
   `UC.Quantitative.Family.Compose`, where the two live. FIXED.
