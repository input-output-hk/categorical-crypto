...
|---|---|---|
| B2-M1 | `compose-settles` / `Settles-∘`: identify `𝒢ₚ`'s `_∘_` with a `traceᴹ` whose step has a kernel, so `trace-settles` applies to `real 𝒫.∘ resource` and to `subᴵ simulator 𝒫.∘ (ideal 𝒫.∘ resource)` | MISSING — grep over `src/`: zero occurrences of either name. This is `docs/fcom-uc.md` §"1b" and is the LARGEST item |
| B2-M2 | `Ranked` instances at `real`, `ideal`, `subᴵ simulator`, and the two composites | MISSING — `Ranked` (`Iter.agda:171`) has no instance anywhere under `Examples/`. `docs/fcom-uc.md`:96-101 gives the intended rank (3 values, no arithmetic) but does not write it |
| B2-M3 | the example's two `StepBisim⊥`s against pruned kernels, with `deadR`/`deadI` naming the off-protocol activations | MISSING — no `deadR`, `deadI`, `respR⊥`, `respI⊥`, `prune-adv`, `prune-bound` in `src/`. Only the TOTAL `stepR`/`stepI` exist (`Game.agda:306,360`) |
| B2-M4 | `binding-bound`: the two-sided `cum` bound between the real and simulated-ideal CLOSED machines at every `asks≤ m d` | MISSING — zero occurrences. Its route is B2-M1 → `rawPr` → B2-M3 → `hop-bound⊥` at `prune-cert Game.cert` |
| B2-M7 | the ask-preserving relabelling `Neg (Advᴵ ⊗ᴵ Honᴵ) ↔ Game.Q`, `Pos (Advᴵ ⊗ᴵ Honᴵ) ↪ Game.R` | MISSING. `Q` (`Game.agda:60`) is `askQ`/`comQ`/`opnQ` where `Neg (Advᴵ ⊗ᴵ Honᴵ) = AdvQ ⊎ ⊥` is `queryᴬ`/`commitᴬ`/`openᴬ`; `R` (`:66`) has the extra `idleR`, which is precisely where the machines diverge (B2-M3) |
