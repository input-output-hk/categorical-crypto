...
| `…UC.Seam` | model | historical 171 | `strategyEnv` (a strategy as an environment), `ctxRunˢ`/`runˢ`, `Agreeˢ`, `Adequacy`/`AgreeToAdv` (stated here, both inhabited below), `pov-carry` (proved).  All but the embedding, the two runs and `Adequacy` were retired 2026-09-21 (`end-to-end.md` §4) |
...
| `…UC.Seam.Carry` | model | historical 96 | `run-agree`, `agree-to-adv`, and the now-closed `agreeToAdv`/`povCarry` — all four retired 2026-09-21, leaving `adv-at`/`adv-from-runs` |
| `…UC.Seam.Grounding` | model | historical 152 | `StratIsEnv`, its reduction `EnvCtx`/`EnvAsCtx`, and the trivial grade's `SubBlind`/`IotaBlind`/`UnitGrade` — the grading-dependent statements, read in the INHERITED metatheory at the sealed bundle.  Only `SimTotal`/`SubBlind` survive the 2026-09-21 ruling |
