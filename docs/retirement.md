...
`UC.Model.Family.Negligible`'s whole body is one parameterized application at
the model, the same shape as `UC.Seam.Audit`'s and kept for the same reason.
