...
and subsumption maps satisfying graded Kleisli laws.  The graded
Kleisli category $\mathrm{Kl}_T(\mathcal{C})$ has objects $(u, A)$
and morphisms $(i, A) \to (j, B)$ triples $(k,\,
f : A \to T_k B,\, \alpha : i \otimes k \to j)$.

\paragraph{Applied here.}  We grade $G\SFunM$ by the terminal
monoidal category $\mathbf{1}$ and pick $T_\star = \id$, so
$\mathrm{Kl}_T(G\SFunM) \cong G\SFunM$.  This layer is a slot into
which a richer graded monad (e.g.\ ``optional output'' via $\Maybe$
or ``trace history'' via $\mathrm{List}$) can be substituted later
without touching the steps below.

In Agda the abstract construction is \texttt{Categories.GradedKleisli};
the instantiation here is
\texttt{Machine.Category.SFunᵉ-GradedKleisli} with the trivial
\texttt{SFunᵉ-GradedTriple}.
...
\begin{itemize}
  ...
  \item the graded Kleisli (trivial identity grading):
        \texttt{Categories.GradedKleisli} applied via
        \texttt{Machine.Category.SFunᵉ-GradedKleisli} (Step 5).
\end{itemize}
