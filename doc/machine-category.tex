...
grading $\mathcal{V} = \mathbf{1}$ and $T_\star = \id$, so
$\mathrm{Kl}_T(G\SFunM) \cong G\SFunM$.  The point is purely
prospective: the layer is a substitution slot.  Replacing the
trivial triple with, e.g., a $\Maybe$-graded one would absorb the
$\Maybe$-bookkeeping of $\mathrm{MaybeHom}$ into the graded Kleisli
machinery, discharging several category-law obligations at once.
