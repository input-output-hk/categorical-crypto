...
\begin{itemize}
  ...
  \item an \emph{iteration} operator
        ...
        naturality, parameter, codiagonal, uniformity);
...
\begin{lemma}
$\SFunM$ is a category.  The proofs reduce to the monad laws and
extensionality of $M$.
\end{lemma}

In Agda this is \texttt{CategoricalCrypto.SFunM.SFunᵉ-Category}.
...
\begin{lemma}
$(\sqcup, \bot)$ exhibits $\SFunM$ as a symmetric monoidal category.
Associator, unitors and braiding are \emph{pure reshapes}
(stateless deterministic morphisms) of the underlying coproduct
structure on $\mathbf{Type}$, lifted to $\SFunM$ via $\ret$.
\end{lemma}

The pentagon, triangle, hexagon and braiding-involution coherences
all reduce, via $\bind$/$\ret$ laws and extensionality, to ordinary
coherence in $(\mathbf{Type}, +, 0)$.

In Agda this is \texttt{CategoricalCrypto.SFunM.Monoidal.SFunᵉ-monoidal}
(together with its symmetric extension in \texttt{Traced.agda}).
...
Vanishing-1 and yanking follow from $\mathrm{iter}$-fixpoint plus monad
laws.  Vanishing-2 and superposing rely on
\emph{$\mathrm{iter}$-codiagonal} and \emph{$\mathrm{iter}$-uniformity}~\cite{Hasegawa97}.

In Agda this is \texttt{CategoricalCrypto.SFunM.Traced.SFunᵉ-traced}.

\subsection{Step 4: the G-construction (``Int'' / geometry of interaction)}

Given a traced symmetric monoidal category $(\mathcal{C}, \otimes, I,
\mathrm{tr})$, the
\emph{Joyal--Street--Verity G-construction}~\cite{JSV} produces a
compact-closed category $G\mathcal{C}$:
\begin{itemize}
  \item $\Obj(G\mathcal{C}) = \Obj(\mathcal{C}) \times \Obj(\mathcal{C})$;
        write objects as pairs $(A^+, A^-)$;
  \item $\Hom_{G\mathcal{C}}((A^+, A^-), (B^+, B^-)) =
         \Hom_\mathcal{C}(A^+ \otimes B^-,\, A^- \otimes B^+)$;
  \item identity at $(A^+, A^-)$: the braiding
        $\sigma : A^+ \otimes A^- \to A^- \otimes A^+$;
  \item composition: given
        $f : A^+ \otimes B^- \to A^- \otimes B^+$ and
        $g : B^+ \otimes C^- \to B^- \otimes C^+$, set
        \[
          g \circ_G f = \mathrm{tr}_{B^+ \otimes B^-}\bigl(
            \alpha \circ (f \otimes g) \circ \gamma\bigr),
        \]
        where $\alpha, \gamma$ are the symmetry-and-associator
        isomorphisms that arrange the tensors so the shared
        $B$-component can be traced out.
\end{itemize}

\begin{theorem}[\cite{JSV}]
If $\mathcal{C}$ is traced symmetric monoidal, then $G\mathcal{C}$ is
a compact-closed category and the assignment
$A \mapsto (A, I)$ is a strong monoidal functor $\mathcal{C} \to G\mathcal{C}$.
\end{theorem}

\paragraph{Applied here.}  Take $\mathcal{C} = \SFunM$ with
$(\otimes, I) = (\sqcup, \bot)$.  Then
$G\SFunM$ has objects pairs of types $(A^+, A^-)$ and homs
\[
...
typed-channel shape used by \texttt{Machine.Core}.

In Agda this is \texttt{CategoricalCrypto.Machine.Category.SFunᵉ-GConstruction}.

\subsection{Step 5: graded Kleisli over $G\SFunM$}

A \emph{graded monad}~\cite{Katsumata} on $\mathcal{C}$ graded by a
monoidal category $\mathcal{V}$ assigns to each object $u \in \mathcal{V}$
a functor $T_u : \mathcal{C} \to \mathcal{C}$, with a Kleisli extension
\[
  \mathrm{ext}_u : (A \to T_v B) \to (T_u A \to T_{u \otimes v} B)
\]
and subsumption maps $\mathrm{sub}_\alpha : T_u \Rightarrow T_v$ for each
$\alpha : u \to v$ in $\mathcal{V}$, satisfying graded Kleisli laws.
Its \emph{graded Kleisli category} $\mathrm{Kl}_T(\mathcal{C})$ has
\begin{itemize}
  \item objects: pairs $(u, A)$ with $u \in \mathcal{V}$ and
        $A \in \mathcal{C}$;
  \item morphisms $(i, A) \to (j, B)$: triples $(k,\,
        f : A \to T_k B,\, \alpha : i \otimes k \to j)$;
  \item composition reuses $\mathrm{ext}$ and the graded subsumption.
\end{itemize}

\paragraph{Applied here.}  We grade $G\SFunM$ by the \emph{terminal}
monoidal category $\mathbf{1}$ (single object $\star$, single
morphism, $\star \otimes \star = \star$).  This collapses graded
Kleisli to ordinary Kleisli for whichever monad $T_\star$ we choose.
We pick the \emph{identity monad}: $T_\star A = A$.  Then
$\mathrm{Kl}_T(G\SFunM)$ is canonically isomorphic to $G\SFunM$
itself.

The point of this layer is to expose a slot into which a richer
graded monad (e.g.\ encoding ``optional output'' via $\Maybe$, or
``trace history'' via $\mathrm{List}$) can be substituted later
without touching the steps below.

In Agda this is \texttt{CategoricalCrypto.Machine.Category.SFunᵉ-GradedKleisli}
together with the concrete identity triple \texttt{SFunᵉ-GradedTriple}.
...
and ``messages flowing out''.  The transpose
$A^T = (\mathrm{out}_A, \mathrm{in}_A)$ swaps directions, and the
tensor $A \otimes B$ takes coproducts component-wise.
...
input message, optional output message, next state).
...
\noindent
The horizontal arrows are: Kleisli of $M$; equipping with the
coproduct tensor; taking the JSV trace; the $G$-construction; the
graded Kleisli over the identity graded monad (which collapses).

The horizontal arrows are well-known constructions on a category:
Kleisli of a monad, equipping with a coproduct tensor, taking a
trace, $G$-construction, and graded Kleisli.  Each is functorial and
preserves the structure used by the next.  The final bijection
$\mathrm{MaybeHom} \cong \Machine$ uses the
$\mathrm{MonadOfRel}$ bridge to absorb the
...
\begin{itemize}
  ...
  \item \emph{Superposing} and \emph{vanishing-2} for $\SFunM$.
        Derivable from $\mathrm{iter}$-codiagonal and
        $\mathrm{iter}$-uniformity.
  ...
  \item The Machine-extensionality form of the round-trip
        ...
        logical equivalence).
  \item Whether the bijection-induced $\mathrm{MaybeHom}$
        composition agrees, definitionally, with a structurally
        defined Kleisli composition over the Maybe-graded triple.
        Replacing the trivial graded triple with the Maybe-graded one
        would discharge the four $\mathrm{MaybeHom}$ category-law
        obligations in one stroke.
\end{itemize}
...
\[
  \mathrm{tr}^X_{A, B} : \mathcal{C}(A \otimes X,\, B \otimes X)
                       \to \mathcal{C}(A, B),
\]
called the \emph{trace}, satisfying four axioms~\cite{JSV}:
\begin{itemize}
...
\paragraph{Intuition.}
Think of morphisms as boxes with $\otimes$-products of wires entering
on the left and leaving on the right.  Tracing $X$ joins one
$X$-output back to one $X$-input --- a \emph{feedback loop}.  The
axioms say loops can be slid around (naturality), nested (vanishing),
moved past parallel boxes (superposing), and a wire crossed and
immediately re-routed to itself is a straight wire (yanking).  This
``string diagram'' picture is rigorous: \cite{JSV} (see also
\cite{Selinger} for a survey) show
that two terms denote equal morphisms iff their string diagrams are
isotopic in the plane (with appropriate boundary conditions).
...
Being compact closed means every object $(A^+, A^-)$ has a
\emph{dual}, $(A^+, A^-)^* = (A^-, A^+)$, with unit and counit
morphisms (``cup'' and ``cap''):
\[
