...
(\texttt{Machine.Iso}) with respect to machine isomorphism
$\cong^{M}$, an equality coarser than propositional equality, which
serves as the hom-equality of $\Machine$.  The same bisimulations
...
$\Machine$ carries the hom-equality $\cong^{M}$.  Its laws are
exactly Theorem~\ref{thm:bisim}, and congruence of composition is
\texttt{∘-resp-≅ᴹ}.  The Agda instance is
\texttt{Machine.Category.MachineCategory}.
\end{construction}
